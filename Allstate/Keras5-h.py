...
skewed_feats = train[f_num].apply(lambda x: skew(x.dropna()))
# transform features with skew > 0.25 (this can be varied to find optimal value)
skewed_feats = skewed_feats[skewed_feats > 0.2]
skewed_feats = skewed_feats.index
for feats in skewed_feats:
    tr_te[feats] = tr_te[feats] + 1
    tr_te[feats], lam = boxcox(tr_te[feats])
...
for (inTr, inTe) in folds:
    ...
    pred /= nbags
    pred_oob[inTe] = pred
    score = mean_absolute_error(np.exp(yte) - shift , np.exp(pred)-200)
    i += 1
    print('Fold ', i, '- MAE:', score)

print('Total - MAE:', mean_absolute_error(np.exp(y) - shift , np.exp(pred_oob)-200))

## train predictions
df = pd.DataFrame({'id': id_train, 'loss': np.exp(pred_oob) - shift})
df.to_csv('preds_oob.csv', index = False)

## test predictions
pred_test /= (nfolds*nbags)
df = pd.DataFrame({'id': id_test, 'loss':  np.exp(pred_test)-shift})

...
train = pd.read_csv('train.csv', dtype={'id': np.str, 'loss': np.float32})
y = np.array(train['loss'])
test = pd.read_csv('test.csv', dtype={'id': np.str})
...
rfr = RandomForestRegressor(n_estimators=500, max_features='sqrt', max_depth=20, n_jobs=4)
